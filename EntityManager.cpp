	...
	)
{
	...
	CEntity* newEntity = new CTankEntity(patrolList, tankTemplate, m_NextUID, team,maxSpeed, acceleration, turnSpeed, turretTurnSpeed, maxHP,shellDamage, name, position, rotation, scale);
	...
	)
{
	...
	// Create new tank entity with next UID
	CEntity* newEntity = new CShellEntity(entityTemplate, m_NextUID,damage,
...
TEntityUID CEntityManager::CreateAmmoCrate
(
	const string& templateName,
	const string& name /*= ""*/,
	const CVector3& position /*= CVector3::kOrigin*/,
...
)
{
	...
	// Create new tank entity with next UID
	CEntity* newEntity = new CAmmoCrateEntity(entityTemplate, m_NextUID,
		name, position, rotation, scale);
...
// a helper method which returns a vector containing ALL tanks currently alive
std::vector<TEntityUID> CEntityManager::GetList(string listOf)
{
	std::vector<TEntityUID> m_List;

	/////////////////////////// get list of all tanks
	BeginEnumEntities("", "", listOf);
	CEntity* entity = EnumEntity();
	m_List.clear();

	while (entity != 0) //runs until "EntityManager.EnumEntity();" has no more matching entities
	{
		CTankEntity* tank = dynamic_cast<CTankEntity*>(entity);

		m_List.push_back(tank->GetUID());
...
// a helper method which returns a vector containing ALL tanks currently alive whch are not on the given team
std::vector<TEntityUID> CEntityManager::GetEnemiesList(TUInt32 team)
{
	std::vector<TEntityUID> m_EnemiesList;
	...
	while (entity != 0) //runs until "EntityManager.EnumEntity();" has no more matching entities
	{
		...
		{
			m_EnemiesList.push_back(tank->GetUID());
		}
